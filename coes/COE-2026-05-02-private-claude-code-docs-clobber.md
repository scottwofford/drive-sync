...
| Add a post-sync correctness check: if `LOCAL_DIR` is a git repo, run `git status --porcelain` after sync and `notify_error` if previously-clean tree now has tracked-file modifications | (Trello card to be filed by Scott — see below) | Follow-up PR | After implementation, manually trigger an old-conf scenario (rename Drive copy to be older) → expect macOS notification "Drive Sync clobbered tracked files in <path>" | Detection | Pending Trello |
| Audit `automation/` and any other local rclone-using scripts for the same bidirectional-on-same-remote pattern | (Trello card to be filed by Scott) | Follow-up PR or "no action" note | Audit doc lists all rclone configs touched, marks each as "safe / vulnerable / fixed" | Detection | Pending Trello |

*Requires human decision/design:*

| Action | Trello card | Delivering PR / artifact | Success criteria | Type | Status |
|--------|-------------|--------------------------|-----------------|------|--------|
| Decide whether reverse-syncing private-claude-code-docs to Google Drive is still useful, given that `claude.ai` web reads the GitHub-synced repo directly. If not, delete the reverse sync entirely (and the launchd job) rather than leaving it as one-direction-only | (Trello card to be filed by Scott) | Decision + (optional) PR removing the launchd job | Either: (a) Trello card moves to DONE with "kept; reason: X" comment, or (b) launchd plist deleted and `private-claude-code-docs.conf` removed | Architectural | Human decision |
| Decide on alerting/observability for this class of "silent success that destroys data" bug across ALL drive-sync targets, not just patched configs | (Trello card to be filed by Scott) | Decision document | A brief in `coes/` or `README.md` describing the chosen detection mechanism (post-sync verification, periodic git divergence check, Healthchecks.io ping, etc.) | Detection | Human decision |

**Trello cards:** the Claude session attempted to file these via the trello MCP but did not have the cards' shortlinks at write time. Scott to file the four follow-up cards on the Luthien board (or a personal board if drive-sync isn't tracked there) and substitute the shortlinks into this table on a follow-up commit. Tracked under "do this before merging the PR" on the PR description.
